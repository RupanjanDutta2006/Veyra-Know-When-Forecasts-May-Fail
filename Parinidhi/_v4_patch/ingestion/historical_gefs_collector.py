...
class HistoricalGEFSCollector:
    ...
    def collect_run(
        ...
        for member in actual_members:
            ...
            for var in variables:
                ...
                for prefix in prefixes:
                    ...
                    H = Herbie(
                        issue.to_pydatetime(), model="gefs_reforecast", fxx=max(1, int(horizon_hours)),
                        member=member, variable_level=cfg["file"], priority=["aws"], verbose=False,
                    )
                    ds = H.xarray(cfg["search"], backend_kwargs={"indexpath": ""})
                    datasets[prefix] = _point(ds, latitude, longitude)
                    if grid_lat is None:
        ...
        manifest = {
            ...
            "source_urls": sorted({
                _url(issue, member, VARIABLES[prefix]["file"])
                for member in actual_members
                for prefix in (variables if variables else ())
