...
def _open_member_field(issue: pd.Timestamp, member: int, cfg: Dict, raw_dir: Path,
    ...
    """Use Herbie with an explicit project cache; fall back to exact NOAA URL for extended members."""
    if member <= 4:
        from herbie import Herbie
        H = Herbie(issue.tz_localize(None).to_pydatetime(), model="gefs_reforecast", fxx=max(1, int(fxx)),
                   member=member, variable_level=cfg["file"], priority=["aws"], save_dir=str(raw_dir / "herbie"), verbose=False)
