...
def _open_member_lead(issue: pd.Timestamp, member: int, cfg: Dict, raw_dir: Path, latitude: float, longitude: float, lead: int):
    """Fetch exactly one forecast lead using a project-controlled Herbie subset.

    We deliberately use H.download(search=...) and then open the returned local
    path.  This avoids Herbie's xarray subset-cache path handling, which can
    produce stale/nonexistent relative paths on Windows.
    """
    if member <= 4:
        from herbie import Herbie
        save_dir = (raw_dir / "herbie").resolve()
        H = Herbie(
            issue.tz_localize(None).to_pydatetime(),
            model="gefs_reforecast",
            fxx=max(1, int(lead)),
            member=member,
            variable_level=cfg["file"],
            priority=["aws"],
            save_dir=save_dir,
            verbose=False,
        )
        ...
        import xarray as xr
        ds = xr.open_dataset(local_path, engine="cfgrib", backend_kwargs={"indexpath": ""})
        return _point(ds, latitude, longitude), str(H.grib)

    # Extended weekly members are not exposed by the current Herbie template.
    # Keep the exact NOAA URL as the fallback and cache the byte-for-byte GRIB2.
...
class HistoricalGEFSCollector:
    ...
    def collect_run(
        ...
        per_member: Dict[int, Dict[str, Dict[int, Dict[str, object]]]] = {}
        for member in actual_members:
            per_member[member] = {}
            for var in variables:
                prefixes = ("u_wind_10m", "v_wind_10m") if var == "wind_speed_10m" else (var,)
                lead_datasets: Dict[int, Dict[str, object]] = {}
                for lead in wanted_leads.astype(int):
                    lead_datasets[lead] = {}
                    for prefix in prefixes:
                        cfg = VARIABLES[prefix]
                        ds, source_url = _open_member_lead(
                            issue, member, cfg, self.raw_dir, latitude, longitude, int(lead)
                        )
                        lead_datasets[lead][prefix] = ds
                        source_urls_seen.add(source_url)
                        if grid_lat is None:
                            grid_lat = float(ds.latitude.values)
                            grid_lon = float(ds.longitude.values)
                per_member[member][var] = lead_datasets
        ...
        for var in variables:
            for lead in wanted_leads.astype(int):
                ...
                for member in actual_members:
                    dsmap = per_member[member][var][lead]
                    if var == "wind_speed_10m":
                        u, v = dsmap["u_wind_10m"], dsmap["v_wind_10m"]
                        actual_valid = pd.Timestamp(u.valid_time.values)
                        if actual_valid.tzinfo is None:
                            actual_valid = actual_valid.tz_localize("UTC")
                        else:
                            actual_valid = actual_valid.tz_convert("UTC")
                        if abs((actual_valid - valid).total_seconds()) > 60:
                            raise ValueError(f"No exact GEFS lead for {valid}; received {actual_valid}")
                        uval = float(np.asarray(u["u10"].values).reshape(-1)[0])
                        vval = float(np.asarray(v["v10"].values).reshape(-1)[0])
                        value = math.sqrt(uval * uval + vval * vval) * 3.6
                    else:
                        ds = dsmap[var]
                        actual_valid = pd.Timestamp(ds.valid_time.values)
                        if actual_valid.tzinfo is None:
                            actual_valid = actual_valid.tz_localize("UTC")
                        else:
                            actual_valid = actual_valid.tz_convert("UTC")
                        ...
                        value = float(np.asarray(ds[data_var].values).reshape(-1)[0])
